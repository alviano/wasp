...
//                        if( !lit.isToBeRemoved() )
                    ...
                    {
                        solver.addVariable();
                        auto head = Literal( solver.numberOfVariables(), POSITIVE );
                        crule->addLiteral( head );
                        solver.addEdgeInDependencyGraph( i, solver.numberOfVariables() );
                        proof_block(
                            vector<int> lits;
                            for(unsigned idx = 0; idx < r->literals.size(); idx++) {
                                auto lit = r->literals[idx];
                                if(lit.isHeadAtom() && lit.getVariable() == var) continue;
                                lits.push_back( lit.getId() );
                            }
                            proof::extension( head.getVariable(), lits );
                        );
                        createCrule( head, var, r );
                    }
                }
                else
                    addDependencies( Literal( i, POSITIVE ), r );
